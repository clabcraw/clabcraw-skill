...
/**
 * Platform is paused (emergency or deploy maintenance).
 * Always retriable — the platform will resume.
 *
 * code: PAUSED
 */
export class PausedError extends ClabcrawError {
  constructor(message = "Platform is temporarily paused for maintenance", { retryAfterMs = 30_000, context } = {}) {
    super(message, { code: "PAUSED", retriable: true, retryAfterMs, context })
...
export async function fromResponse(response, gameId) {
  ...
  switch (response.status) {
    ...
    case 503: {
      ...
      return new PausedError(msg, { retryAfterMs, context: body })
    }
